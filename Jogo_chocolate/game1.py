...
class Background:
    def __init__(self, surface, b_type, x=0, right=False):
        ...
        self.b_type = b_type
        if b_type == "margin":
            files = ["umpalumpas.png", "ovo.png", "Caminho de biscoito.png", "arvore.png"]
...
class Chocolate:
    ...
    def __init__(self, interval, screen_width, screen_height, choc_type):
        ...
        self.image = pg.image.load("nestle.png")
        if self.choc_type == "fast":
            self.image = pg.image.load("alpino.png")
            self.speed += 4
        elif self.choc_type == "big":
            self.image = pg.image.load("toblerone.png")
            self.speed -= 2
            self.width = 150
            self.height = 50
...
class PowerUp:
    ...
    def __init__(self, interval, screen_height, pwu_type):
        ...
        self.image = pg.image.load("cenoura.png")
        self.collision_rect = Rect((self.x - self.width / 2, self.y - self.height / 2, self.width, self.height))
        self.time = 0
...
class Game:
    ...
    def __init__(self):
        ...
        self.score = 0
        self.score_bar = self.font.render('Score:' + str(self.score), True, [0, 0, 0])
    ...
    def actions(self):
        ...
        self.score_bar = self.font.render('Score:' + str(max(0, self.score - 2)), True, [0, 0, 0])
        for chocolate in self.chocolates:
    ...
    def draw(self):
        ...
        self.screen.blit(self.score_bar, (20, 5))
        pg.display.flip()
